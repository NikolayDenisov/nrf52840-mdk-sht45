...
#define SHT4X_COMMAND_READ_SERIAL_NUMBER                                       \
  0x89 /**< read serial number command                                         \
        */
 ...
 * @note       none
 */
static uint8_t a_sht4x_write_read(sht4x_handle_t *handle, uint8_t cmd,
                                  uint16_t delay, uint8_t *data, uint16_t len) {
...
uint8_t sht4x_init(sht4x_handle_t *handle) {
  ...
  res = a_sht4x_write_read(handle, SHT4X_COMMAND_SOFT_RESET, 10, NULL,
                           0); /* soft reset */
...
uint8_t sht4x_read(sht4x_handle_t *handle, sht4x_mode_t mode,
  ...
  {
  case SHT4X_MODE_HIGH_PRECISION_WITH_NO_HEATER: /* measure T & RH with high
                                                    precision */
  {
    res = a_sht4x_write_read(handle, SHT4X_MODE_HIGH_PRECISION_WITH_NO_HEATER,
                             10, buf, 6); /* read data */
    if (res != 0)                         /* check result */
    {
      handle->debug_print(
  ...
  case SHT4X_MODE_MEDIUM_PRECISION_WITH_NO_HEATER: /* measure T & RH with medium
                                                      precision */
  {
    res = a_sht4x_write_read(handle, SHT4X_MODE_MEDIUM_PRECISION_WITH_NO_HEATER,
                             5, buf, 6); /* read data */
    if (res != 0)                        /* check result */
    {
      handle->debug_print(
  ...
  case SHT4X_MODE_LOWEST_PRECISION_WITH_NO_HEATER: /* measure T & RH with the
                                                      lowest precision */
  {
    res = a_sht4x_write_read(handle, SHT4X_MODE_LOWEST_PRECISION_WITH_NO_HEATER,
                             2, buf, 6); /* read data */
    if (res != 0)                        /* check result */
    {
      handle->debug_print(
  ...
  case SHT4X_MODE_HIGH_PRECISION_WITH_HEATER_200MW_1S: /* activate heater with
                                                          200mW for 1s, high
                                                          precision */
  {
  ...
  case SHT4X_MODE_HIGH_PRECISION_WITH_HEATER_200MW_0P1S: /* activate heater with
                                                            200mW for 0.1s, high
                                                            precision */
  {
    res = a_sht4x_write_read(handle,
  ...
  case SHT4X_MODE_HIGH_PRECISION_WITH_HEATER_110MW_1S: /* activate heater with
                                                          110mW for 1s, high
                                                          precision */
  {
    res = a_sht4x_write_read(handle,
  ...
  case SHT4X_MODE_HIGH_PRECISION_WITH_HEATER_110MW_0P1S: /* activate heater with
                                                            110mW for 0.1s, high
                                                            precision */
  {
    res = a_sht4x_write_read(handle,
  ...
  case SHT4X_MODE_HIGH_PRECISION_WITH_HEATER_20MW_1S: /* activate heater with
                                                         20mW for 1s, high
                                                         precision */
  {
    res = a_sht4x_write_read(handle,
  ...
  case SHT4X_MODE_HIGH_PRECISION_WITH_HEATER_20MW_0P1S: /* activate heater with
                                                           20mW for 0.1s, high
                                                           precision */
  {
    res = a_sht4x_write_read(handle,
  ...
  *temperature_raw =
      (uint16_t)((((uint16_t)buf[0]) << 8) | buf[1]); /* get raw temperature */
  *humidity_raw =
      (uint16_t)((((uint16_t)buf[3]) << 8) | buf[4]); /* get raw humidity */
  *temperature_s = (float)(*temperature_raw) / 65535.0f * 175.0f -
                   45.0f; /* convert raw temperature */
  *humidity_s = (((float)(*humidity_raw) / 65535.0f) * 125.0f -
                 6.0f);       /* convert raw humidity */
...
uint8_t sht4x_get_serial_number(sht4x_handle_t *handle, uint8_t num[4]) {
  ...
  res = a_sht4x_write_read(handle, SHT4X_COMMAND_READ_SERIAL_NUMBER, 10, buf,
 ...
 * @note       none
 */
uint8_t sht4x_write_read(sht4x_handle_t *handle, uint8_t cmd, uint16_t delay_ms,
  ...
  return a_sht4x_write_read(handle, cmd, delay_ms, data,
                            len); /* write and read */
}
